...
To make the server safe to hand to a team, the tools gate access so a call is exposed only when it is a **customer-facing sales/CS call** — everything else (exec syncs, HR, M&A, strategy, vendor/board calls, anything private) is invisible: dropped from `list_calls`, refused by `retrieve_transcripts` and `get_call_highlights`, and skipped by `search_call_transcripts` before any transcript is fetched. Two signals define "customer-facing"; use **either or both** (with neither set, the gate is off and everything is exposed):

**Recommended — `GONG_REQUIRE_CRM_ACCOUNT=true` (a known CRM account, no roster to maintain).** A call is in scope only when a customer/prospect actually *spoke* on it **and** Gong has linked it to a **CRM account**. This defines "customer" straight from your CRM (via Gong's CRM integration) and stays current on its own — there's no email list to maintain and no managers to exclude, because internal/leadership calls aren't tied to a customer account, and a board member / investor / advisor / vendor isn't a CRM account either, so all of those stay hidden. **Verify before enabling:** run a `search_call_transcripts` and confirm the matches carry an `accounts` field; if it's empty, Gong isn't returning CRM context and this would hide everything. Fails safe (off until set).

**Optional — an explicit roster (`GONG_TEAM_USER_IDS` / `GONG_TEAM_EMAILS`).** A comma-separated list of your sales/CS/SDR/support reps; when set, a call must also include a roster member. Use it to scope to GTM-team calls specifically, or as a fallback when CRM context isn't available. If you rely on the roster *without* the CRM requirement, list the reps and **not** leadership — that's what keeps a CRO↔vendor or exec↔investor call out (no rostered rep on it). User IDs are the stable match; emails are easier to maintain.
