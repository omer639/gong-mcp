...
To make the server safe to hand to a team, set a **roster** — `GONG_TEAM_USER_IDS` and/or `GONG_TEAM_EMAILS` (comma-separated). When either is set, every tool enforces one rule: a call is exposed only when it is a **customer-facing sales/CS call**, defined as **a roster member is on it AND a customer/prospect actually _spoke_ on it**. Anything else — exec syncs, HR, M&A, strategy, vendor calls, anything private the team wasn't on — is invisible: dropped from `list_calls`, refused by `retrieve_transcripts` and `get_call_highlights`, and skipped by `search_call_transcripts` before any transcript is fetched.

- **List the actual reps — not leadership.** The roster is what excludes leadership/vendor calls: a CRO↔vendor or exec↔investor call has a real external *speaker*, but with the CRO off the roster it has no rostered rep, so it stays out. Put a leader on the roster only if you want their external calls visible to everyone.
- **"Spoke", not "invited", is deliberate.** Internal meetings (a revenue sync, a churn review) often carry a silent external invitee — an advisor, a calendar guest, a note-taker — which would make a plain "has an external participant" check pass. Requiring a customer who actually spoke drops those while keeping real customer calls, where the customer always talks.
- **Optional CRM hardening — `GONG_REQUIRE_CRM_ACCOUNT`.** One gap remains after the above: an internal/leadership call where a *non-customer* external — a board member, investor, advisor, or vendor — actually speaks alongside a rostered manager. Participant affiliation can't tell that person from a customer. Set `GONG_REQUIRE_CRM_ACCOUNT=true` to also require the call to be linked to a **CRM account** (Gong auto-links calls to CRM companies/deals): a board/advisor call has no customer account, so it drops, while real customer calls stay. **Verify before enabling:** it depends on Gong being integrated with your CRM and returning account context — run a `search_call_transcripts` and confirm the matches carry an `accounts` field; if they don't, turning this on hides everything. It **fails safe** by being off until explicitly set.
- **User IDs** are the stable match; **emails** are easier to maintain. Set either or both.
...
- **Limits worth knowing:** the roster can't express "rep A sees less than rep B" — everyone connected gets the same team-scoped view. And a private but genuinely customer-facing call a rep spoke on with a customer is still visible to the whole team. Per-person visibility matching each user's own Gong permissions would require per-user Gong OAuth, which this server does not implement. With the roster **unset, the gate is off** and every call the key can see is exposed — so set it before serving more than yourself.
